...
def list_series(series,seriesname,fanart_image):
    ...
    listing = []
    for serie in soup_series.find_all("div",attrs={"class":"col-lg-1 col-md-2 col-sm-4 col-xs-4"}):
        if not serie.a:
            continue
        list_item = xbmcgui.ListItem(label=serie.a.get_text().replace(" ","").replace("\n",""))
        if not serie.a.get("href").startswith("/"):
            continue
        url = '{0}?action=play_video&episode={1}&name={2}'.format(_url, serie.a.get("href"), \
                      "[COLOR green]" +  seriesname + "[/COLOR] [COLOR yellow]" + serie.a.get_text().replace(" ","").replace("\n","").encode("utf-8") + "[/COLOR]")
        is_folder=False
        listing.append((url, list_item, is_folder))
